...
class particleFilter:
    ...
    def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
        ...
        self.weights = []
        n = self.numParticles
        x = self.curMax[0]*np.random.uniform(0,1,n)
        y = self.curMax[1]*np.random.uniform(0,1,n)
    ...
    def Measurement_Model(self, x, y):
        ...
        ####################################################################
        # Students finish the code below
        robotpostion = self.model.readingSensor(x,y)
        particlepostion = []
        weight = 0
        for i in range(self.numParticles):
            particlepostion.append(self.model.readingMap(self.particles[i]))
            dis = np.sqrt((robotpostion[0]-particlepostion[i][0])**2+(robotpostion[1]-particlepostion[i][1])**2+(robotpostion[2]-particlepostion[i][2])**2+(robotpostion[3]-particlepostion[i][3])**2)
            w_t = np.exp(-dis**2/(2*self.std))
            self.weights[i] = w_t
            weight += w_t
